...
export default function CheckoutModal({ billing, hasUsedTrial, onSuccess, onClose, t }) {
  ...
  if (done) return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100, fontFamily: "'DM Sans', sans-serif" }}>
      <div style={{ background: t.bg, borderRadius: 24, maxWidth: 420, width: "100%", boxShadow: "0 32px 80px rgba(0,0,0,0.25)", padding: "48px 32px", textAlign: "center" }}>
        ...
        <h3 style={{ fontSize: 22, fontWeight: 740, color: t.fg, margin: "0 0 8px" }}>You're all set!</h3>
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1100, padding: 24, fontFamily: "'DM Sans', sans-serif" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: t.bg, borderRadius: 24, maxWidth: 460, width: "100%", boxShadow: "0 32px 80px rgba(0,0,0,0.25)", overflow: "hidden", maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ padding: "24px 28px 18px", borderBottom: `1px solid ${t.borderSoft}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div><p style={{ fontSize: 15, fontWeight: 700, color: t.fg, margin: 0 }}>ReadFlow Pro</p><p style={{ fontSize: 12, color: t.fgSoft, margin: 0 }}>{billing === "monthly" ? "Monthly" : "Annual"} subscription</p></div>
          </div>
          <button onClick={onClose} style={{ background: t.surface, border: "none", cursor: "pointer", color: t.icon, padding: 6, borderRadius: 8 }}><X size={16} /></button>
        </div>
        ...
        <div ref={formRef} style={{ padding: "14px 28px 28px" }}>
          ...
          <button onClick={handleSubmit} disabled={!isValid || processing} style={{ width: "100%", padding: "13px 20px", borderRadius: 12, border: "none", background: isValid && !processing ? t.accent : t.border, color: isValid && !processing ? "#fff" : t.fgSoft, cursor: isValid && !processing ? "pointer" : "not-allowed", fontSize: 14, fontWeight: 670, fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, boxSizing: "border-box", opacity: processing ? 0.8 : 1 }}>
