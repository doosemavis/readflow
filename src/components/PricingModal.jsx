...
export default function PricingModal({ onClose, onSelectPlan, hasUsedTrial, t }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", backdropFilter: "blur(6px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 24, fontFamily: "'DM Sans', sans-serif" }}>
      <div onClick={e => e.stopPropagation()} style={{ background: t.bg, borderRadius: 24, maxWidth: 680, width: "100%", boxShadow: "0 32px 80px rgba(0,0,0,0.25)", overflow: "hidden" }}>
        <div style={{ padding: "28px 32px 20px", borderBottom: `1px solid ${t.borderSoft}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            <h2 style={{ fontSize: 24, fontWeight: 740, color: t.fg, margin: 0, letterSpacing: "-0.02em" }}>Upgrade to ReadFlow Pro</h2>
            <p style={{ fontSize: 14, color: t.fgSoft, margin: "6px 0 0" }}>Unlimited documents, all formats, one reader built for you.</p>
          </div>
          <button onClick={onClose} style={{ background: t.surface, border: "none", cursor: "pointer", color: t.icon, padding: 8, borderRadius: 10 }}><X size={18} /></button>
        </div>
        <div style={{ padding: "20px 32px 0", display: "flex", justifyContent: "center" }}>
          <div style={{ display: "flex", background: t.surface, borderRadius: 12, padding: 4, gap: 2 }}>
            {[{ key: "monthly", label: "Monthly" }, { key: "annual", label: "Annual", badge: "Save 25%" }].map(b => (
              <button key={b.key} onClick={() => setBilling(b.key)} style={{ padding: "8px 20px", borderRadius: 10, border: "none", cursor: "pointer", background: billing === b.key ? (t.bg === "#111116" || t.bg === "#0B0E14" ? t.surfaceHover : "#fff") : "transparent", boxShadow: billing === b.key ? "0 1px 4px rgba(0,0,0,0.1)" : "none", color: billing === b.key ? t.fg : t.fgSoft, fontSize: 13, fontWeight: 600, fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", gap: 6, transition: "all 0.2s" }}>
        ...
        <div style={{ padding: "20px 32px 32px", display: "flex", gap: 16, alignItems: "stretch" }}>
          ...
          <div onClick={() => onSelectPlan(billing)} onMouseEnter={() => setHoverCard("pro")} onMouseLeave={() => setHoverCard(null)} style={{ flex: 1, borderRadius: 16, padding: "24px 20px", border: `2px solid ${hoverCard === "pro" ? t.accent : t.accent + "AA"}`, background: hoverCard === "pro" ? `${t.accent}18` : t.accentSoft, position: "relative", display: "flex", flexDirection: "column", cursor: "pointer", transform: hoverCard === "pro" ? "translateY(-3px)" : "translateY(0)", boxShadow: hoverCard === "pro" ? `0 8px 28px ${t.accent}25` : "none", transition: "all 0.25s cubic-bezier(0.4,0,0.2,1)" }}>
            ...
            <button onMouseEnter={() => setHoverBtn("pro")} onMouseLeave={() => setHoverBtn(null)} onClick={e => { e.stopPropagation(); onSelectPlan(billing); }} style={{ width: "100%", padding: "11px 16px", borderRadius: 10, border: "none", background: hoverBtn === "pro" ? t.fg : t.accent, color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 660, display: "flex", alignItems: "center", justifyContent: "center", gap: 7, marginTop: 12, boxSizing: "border-box", transition: "all 0.2s ease", transform: hoverBtn === "pro" ? "scale(1.02)" : "scale(1)", boxShadow: hoverBtn === "pro" ? `0 4px 12px ${t.accent}40` : "none" }}>
        ...
        <div style={{ padding: "14px 32px 18px", borderTop: `1px solid ${t.borderSoft}`, display: "flex", alignItems: "center", justifyContent: "center", gap: 16 }}>
          {["Cancel anytime", "Secure checkout via Stripe", "No hidden fees"].map((s, i) => <span key={i} style={{ fontSize: 11, color: t.fgSoft, display: "flex", alignItems: "center", gap: 4 }}><Check size={11} style={{ color: t.icon }} /> {s}</span>)}
        </div>
      </div>
    </div>
  );
